  ...
  return (
    <header className="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 cursor-pointer group" onClick={() => onPageChange('apartments')}>
          ...
          <span className="text-xl font-bold tracking-tight text-[#FF385C] group-hover:opacity-80 transition-opacity"
            style={{ fontFamily: "'DM Serif Display', serif" }}>
            mini<span className="text-gray-900">airbnb</span>
          </span>
        </div>
        <nav className="flex gap-1">
          {['apartments', 'reservations'].map(p => (
            <button key={p} onClick={() => onPageChange(p)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${
                currentPage === p ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
              }`}>
              {p === 'apartments' ? 'Stays' : 'My Trips'}
            </button>
          ))}
          {user.role === 'admin' && (
            <button onClick={() => onPageChange('cabinet')}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${
                currentPage === 'cabinet' ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
              }`}>
              Cabinet
            </button>
          )}
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-full cursor-pointer hover:border-gray-400 hover:bg-white transition-all">
